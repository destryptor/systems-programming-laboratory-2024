...
int main(int argc, char *argv[])
{
   ...
   while (1)
   {
      ...
      {
         printf("$ kill(%%%d)\n", OPS->arg1);
         listfree(&T[OPS->arg1]); // REPLACED listkill with listfree
   ...
   // FREEING the individual lists
   for (int i = 0; i <= m; i++)
   {
      listfree(&T[i]);
   }
   // FREEING the list of lists
   free(T);
   // FREEING the list of operations by moving OPS to the beginning of the operations list
   free(OPS - m);
