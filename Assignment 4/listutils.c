// Formatting was auto-changed by built in formatter
#include <stdio.h>
#include <stdlib.h>
#include <listutils.h>
...
slist listadd(slist L, int x)
{
   ...
   while (i < L.n)
   {
      if ((L.item[i] < x) || (inserted))
      { /* insertion position not reached or x is already inserted */
         K.item[j] = L.item[i];
         ++i;
         ++j;
      }
      else
      { /* insert x without consuming the next item from L */
         K.item[j] = x;
         ++j;
         inserted = 1;
      }
   ...
   // HANDLING the case when the data to be added has to be added at the end of the list (greate than all existing elements)
   if (inserted == 0)
      K.item[L.n] = x;
...
slist listsub(slist L, int x)
{
   ...
   if (K.n == 0)
      return (slist){0, NULL}; /* Deletion from a list with only one item */
   ...
   while (i < L.n)
   {
      if ((L.item[i] < x) || (deleted))
      { /* If x is not yet found or deletion is already done */
         K.item[j] = L.item[i];
         ++i;
         ++j;
      }
      else
      { /* Skip copying x from L to K */
         ++i;
         deleted = 1;
      }
   }
...
listop *freadops(char *fname)
{
   ...
   if (fp == NULL)
      return NULL; /* Unable to open input file */
   ...
   for (i = 0; i < m; ++i)
   {
      ...
      if (C[i].operation == LISTINIT)
      { /* No operands to listinit() */
         C[i].arg1 = C[i].arg2 = 0;
      }
      else
      { /* At least once operand for the operation */
